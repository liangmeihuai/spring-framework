 ...
 *以抽象的方式公开引用bean名称的接口。此接口并不一定意味着对实际bean实例的引用;它只是表示一个bean名称的逻辑引用。
 ...
 *
 作为由任何bean引用holder实现的公共接口，例如{@link RuntimeBeanReference RuntimeBeanReference}和
 {@link RuntimeBeanNameReference RuntimeBeanNameReference}.
 *
...
public interface BeanReference extends BeanMetadataElement {
	 ...
	 * 返回这个引用指向的目标bean名称(永远不要@ code null})。
	 */
	String getBeanName();
