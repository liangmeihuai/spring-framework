...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	/**
	 * Load bean definitions from the specified XML file.
	 * 从特定的xml文件中加载beanDefinition
	 * @param encodedResource the resource descriptor for the XML file,
	 * allowing to specify an encoding to use for parsing the file
	 * 允许指定一个编码来解析xml文件的资源描述符
	 * @return the number of bean definitions found 找到的beanDefinition数目
	 * @throws BeanDefinitionStoreException in case of loading or parsing errors 以防加载或者解析时候出错
	 */
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		System.out.println("Loading XML bean definitions from " + encodedResource.getResource());
        //加载Resource资源
		...
		try {
			//从encodeedResource中获取已经封装的Resource对象并再次从Resource中获取其中的inputStream
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				//这里的inputSource是org.xml.sax.InputSource
	 ...
	 * 注册包含在给定的dom 文档中的bean definitions
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		documentReader.setEnvironment(getEnvironment());// set enviroment
		int countBefore = getRegistry().getBeanDefinitionCount();
		// create a XmlReaderContext // TODO: 2017/11/15  to BeanDefinitionDocumentReader
